  ...
  namespace Rpc
  {
    ...
    bool TcpServer::Recv(int fd)
    {
      ...
      if(nb > 0)
      {
        ...
        /* in case of notification message received, the response could be JSON_NULL */
        if(response != JSON_NULL)
        {
    ...
    void TcpServer::WaitMessage(uint32_t ms)
    {
      ...
      if(select(max_sock, &fdsr, NULL, NULL, ms ? &tv : NULL) > 0)
      {
        ...
        for(std::list<int>::iterator it = m_clients.begin() ; it != m_clients.end() ; it++)
        {
          if(FD_ISSET((*it), &fdsr))
          {
            bufferReceive.push_back(*it);
          }
        }

        for(std::vector<int>::iterator it = bufferReceive.begin(); it != bufferReceive.end(); it++)
        {
          Recv((*it));
        }
