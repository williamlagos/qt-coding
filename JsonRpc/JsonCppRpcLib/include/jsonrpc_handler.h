...
namespace Json 
{
  ...
  class Parser
  {
    public:
      virtual ~Parser();
      virtual std::string StringValue(JSONNODE* poNode, std::string strValue);
  };
  /**
   * \namespace Json::Rpc
  ...
  namespace Rpc
  {
    ...
    class CallbackMethod
    {
      public:
         ...
         * \return description
         */
        virtual JSONNODE* GetDescription() const = 0;
    };
    ...
    template<class T> class RpcMethod : public CallbackMethod
    {
      public:
         ...
         */
        typedef bool (T::*Method)(JSONNODE* msg, JSONNODE* response);
         ...
         */
        RpcMethod(T& obj, Method method, const std::string& name, JSONNODE* description = NULL)
        {
          m_obj = &obj;
          m_name = name;
          m_method = method;
         ...
         * \param response response produced (may be JSON_NULL)
         * \return true if message has been correctly processed, false otherwise
         * \note JSON-RPC's notification method MUST set response to JSON_NULL
         */
        virtual bool Call(JSONNODE* msg, JSONNODE* response)
        {
         ...
         * \return description
         */
        virtual JSONNODE* GetDescription() const
        {
    ...
    class Handler
    {
      public:
         ...
         * \param response JSON-RPC response (could be JSON_NULL)
         * \return true if the request has been correctly processed, false otherwise
         * (may be caused by parsed error, ...)
         * \note in case msg is a notification, response is equal to JSON_NULL 
         * and the return value is true.
         */
        bool Process(const std::string& msg, JSONNODE* response);
         ...
         * and the return value is true.
         */
        bool Process(const char* msg, JSONNODE* response);
         ...
         */
        bool SystemDescribe(JSONNODE* msg, JSONNODE* response);

        /**
         * \brief Get a std::string representation of JSONNODE.
         * \param value JSON message
         * \return string representation
         */
        std::string GetString(JSONNODE* value);
      ...
      private:
         ...
         */
        bool Check(JSONNODE* root, JSONNODE* error);

        /**
         * \brief Process a JSON-RPC object message.
         * \param root JSON-RPC message as JSONNODE
         ...
         * and the return value is true.
         */
        bool Process(JSONNODE* root, JSONNODE* response);
    };
