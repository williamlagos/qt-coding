...
Socket::Socket(int type, int protocol, bool nonBlocking) throw(SocketException) {
  #ifdef WIN32
    if (!initialized) {
  ...
  if (m_nonBlocking) {
    ...
    ULONG yes = 1;
    ioctlsocket(sockDesc, FIONBIO, &yes);
  }
}

Socket::Socket(int sockDesc, bool nonBlocking) {
...
CommunicatingSocket::CommunicatingSocket(int type, int protocol, bool nonBlocking)  
    throw(SocketException) : Socket(type, protocol, nonBlocking) {

  m_connected = false;
}

CommunicatingSocket::CommunicatingSocket(int newConnSD, bool nonBlocking) : Socket(newConnSD, nonBlocking) {
...
void CommunicatingSocket::connect(const string &foreignAddress,
  ...
  if (::connect(sockDesc, (sockaddr *) &destAddr, sizeof(destAddr)) < 0) {
    ...
    if (m_nonBlocking && (WSAGetLastError() == WSAEISCONN)) {

      m_connected = true;
      return;
    }

    if (m_nonBlocking && ((WSAGetLastError() == WSAEWOULDBLOCK) || (WSAGetLastError() == WSAEALREADY)))
      return;
    else {

      int nTest = WSAGetLastError();
...
TCPSocket::TCPSocket(const string &foreignAddress, unsigned short foreignPort, bool nonBlocking)
    throw(SocketException) : CommunicatingSocket(SOCK_STREAM, IPPROTO_TCP, nonBlocking) {
  connect(foreignAddress, foreignPort);
}

TCPSocket::TCPSocket(int newConnSD, bool nonBlocking) : CommunicatingSocket(newConnSD, nonBlocking) {
}

// TCPServerSocket Code

TCPServerSocket::TCPServerSocket(unsigned short localPort, int queueLen, bool nonBlocking) 
    throw(SocketException) : Socket(SOCK_STREAM, IPPROTO_TCP, nonBlocking) {
  setLocalPort(localPort);
  setListen(queueLen);
}

TCPServerSocket::TCPServerSocket(const string &localAddress, 
    unsigned short localPort, int queueLen, bool nonBlocking) 
    throw(SocketException) : Socket(SOCK_STREAM, IPPROTO_TCP, nonBlocking) {
...
TCPSocket *TCPServerSocket::accept() throw(SocketException) {
  ...
  return new TCPSocket(newConnSD, m_nonBlocking);
}
