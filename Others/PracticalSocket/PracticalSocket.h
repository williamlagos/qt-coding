...
protected:
  ...
  bool m_nonBlocking;
  Socket(int type, int protocol, bool nonBlocking = false) throw(SocketException);
  Socket(int sockDesc, bool nonBlocking = false);
};
...
protected:
  CommunicatingSocket(int type, int protocol, bool nonBlocking = false) throw(SocketException);
  CommunicatingSocket(int newConnSD, bool nonBlocking = false);
};
...
public:
   ...
   *   @exception SocketException thrown if unable to create TCP socket
   */
  TCPSocket(const string &foreignAddress, unsigned short foreignPort, bool nonBlocking = false) 
      throw(SocketException);

private:
  // Access for TCPServerSocket::accept() connection creation
  friend class TCPServerSocket;
  TCPSocket(int newConnSD, bool nonBlocking = false);
};
...
public:
   ...
   */
  TCPServerSocket(unsigned short localPort, int queueLen = 5, bool nonBlocking = false) 
      throw(SocketException);
  ...
  TCPServerSocket(const string &localAddress, unsigned short localPort,
      int queueLen = 5, bool nonBlocking = false) throw(SocketException);
